...
// GET - Fetch all add-ons (Admin only)
export const GET = withAdminAuth(async (request: NextRequest) => {
  try {
    const externalApiUrl = getExternalApiUrl();
    
    // Forward request to external API
    const response = await fetch(`${externalApiUrl}/addons`, {
      method: 'GET',
      headers: {
    ...
    return createErrorResponse('Failed to fetch add-ons', 500);
  }
});
...
export const PUT = withAdminAuth(async (request: NextRequest) => {
  try {
    ...
    // Forward request to external API
    const response = await fetch(`${externalApiUrl}/addons/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        // Forward any authorization headers if needed
...
export const DELETE = withAdminAuth(async (request: NextRequest) => {
  try {
    ...
    return createApiResponse({ message: 'Add-on deleted successfully' });
  } catch (error) {
